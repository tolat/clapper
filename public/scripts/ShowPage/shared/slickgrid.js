...
createSlickGrid = (data, columns, options) => {
  ...
  grid.onContextMenu.subscribe(function (e, args) {
    ...
    let diffH =
      ...
      (e.pageY + document.getElementById("contextMenu").offsetHeight);
    let diffW =
      document.body.offsetWidth -
      (e.pageX + document.getElementById("contextMenu").offsetWidth);
    if (diffH < 0) {
      e.pageY += diffH;
    }
    if (diffW < 0) {
    ...
    $("#contextMenu").css("top", e.pageY).css("left", e.pageX);
  ...
  grid.onKeyDown.subscribe((e) => {
    if (
      document
        .getElementById("grid-modal-container")
  ...
  grid.onActiveCellChanged.subscribe(function (e, args) {
    ...
    if (
      activeCell &&
      _contextCell &&
      _contextCell.row != activeCell.row &&
      _contextCell.cell != activeCell.cell &&
      !e.shiftKey
    ) {
  ...
  grid.onBeforeEditCell.subscribe(function (e, args) {
    ...
    if (
      ...
      Object.keys(_cellCssStyles["uneditableRow"]).includes(args.item.id)
    ) {
      return false;
    }
  ...
  grid.onClick.subscribe(async function (e, args) {
    ...
    if (
      _activeCell &&
      _cellDblClick &&
      _cellDblClick.row == _activeCell.row &&
      _cellDblClick.cell == _activeCell.cell &&
      !cancelDblClick
    ) {
      grid.editActiveCell();
  ...
  grid.onMouseEnter.subscribe(function (e, args) {
    ...
    setTimeout(function () {
      if (
        cellObject.cellNode == _showContentsHover &&
        cellObject.cellNode.innerText
      ) {
      ...
      for (i = 0; i < values.length; i++) {
        if (
          ...
          values[i]
            .toString()
            .toLowerCase()
            .indexOf(args.searchString.toLowerCase()) >= 0
        ) {
          isMatch = true;
          break;
        }
...
function handleSwipe(e) {
  ...
  function swipe2scroll(e) {
    let scrollX =
      -(e.changedTouches[0].clientX - touchStart.clientX) + _scrollVector.x;
    let scrollY =
      -(e.changedTouches[0].clientY - touchStart.clientY) + _scrollVector.y;
...
function executeSortColumn() {
  _colSortMap[this.field] == 0
    ? _colSortMap[this.field]++
    : _colSortMap[this.field]--;
  ...
  items.sort((a, b) => {
    return stableSort(a, b, asc, this.field);
  });
...
setNavRestrictions = () => {
  ...
  if (!_args.weekid || !_args.weekHasPositions) {
    ...
    crewDropdown.parentElement.replaceChild(
      greyOutLink(crewDropdown),
      crewDropdown
    );
    rentalsDropdown.parentElement.replaceChild(
...
function executePaste() {
  ...
  if (_groupedBy) {
    availableRows = -1;
    let firstItem = (item = items.find(
      (i) => dataView.getRowById(i.id) == this.startRow
    ));
  ...
  for (let i = 0; i <= this.endRow - this.startRow; i++) {
    ...
    if (
      _cellCssStyles["uneditableRow"] &&
      Object.keys(_cellCssStyles["uneditableRow"]).includes(item.id)
    ) {
      continue;
    }
    ...
    for (let j = 0; j <= this.endCell - this.startCell; j++) {
      ...
      if (
        column &&
        (!column.cssClass || !column.cssClass.includes("uneditable"))
      ) {
        // Handle blank cells
...
triggerCopy = () => {
  ...
  for (let i = copyRange.fromRow; i <= copyRange.toRow; i++) {
    ...
    for (let j = copyRange.fromCell; j <= copyRange.toCell; j++) {
      let value = false;
      if (!columns[j].isHidden) {
        value = item[columns[j].field];
      }
      if (value) {
...
reorderColumns = (columnOrder) => {
  ...
  let unaccountedForCols = columns.filter(
    (c) => !newColumns.find((clm) => clm.name == c.name)
  );
...
addRow = (count, direction, isAfterPaste = false) => {
  ...
  editCommand.row = row;
  if (direction == "below") {
    editCommand.direction = 1;
  } else {
    editCommand.direction = 0;
  }
...
function isUneditableCell(item, col) {
  return (
    (col.cssClass &&
      (col.cssClass.includes("uneditable") ||
        col.cssClass.includes("hidden-column"))) ||
    (_cellCssStyles.uneditableRow &&
...
function clearCells() {
  ...
  editCommand.editedfields = cols
    .filter((c) => cols.indexOf(c) >= startCol && cols.indexOf(c) <= endCol)
    .map((c) => c.field);
...
collapseGroups = (collapsedGroups) => {
  ...
  if (groups[0]) {
    for (let i = 0; i < collapsedGroups.length; i++) {
      if (collapsedGroups[i] == 1) {
        dataView.collapseGroup(groups[i].groupingKey);
      } else {
        dataView.expandGroup(groups[i].groupingKey);
      }
    }
  }
};
...
createAutoNumberListener = () => {
  document
    .getElementById("auto-number-input")
...
setFrozenColumns = (numCols) => {
  ...
  let lastFrozenCol = prevCols.find(
    (c) => c.cssClass && c.cssClass.includes("last-frozen-column")
  );
  if (lastFrozenCol) {
    grid.setColumns(
      prevCols.map((c) => {
        if (c.id == lastFrozenCol.id) {
          c.cssClass = c.prevCssClass;
        }
        return c;
      })
    );
...
createFrozenColumnsListener = () => {
  document
    .getElementById("frozen-columns-input")
    .addEventListener("keydown", (e) => {
      if (e.key == "Enter") {
        _frozenColumns =
          parseInt(document.getElementById("frozen-columns-input").value) || 0;
...
toggleAutoNumberModal = (show, group, execute) => {
  ...
  if (!show || !group) {
    ...
    if (execute) {
      group = document
        .getElementById("auto-number-modal-group-display")
        .innerText.slice(1);
      let preserve = document.getElementById(
        "auto-number-modal-preserve-numbers-checkbox"
      ).checked;
      let prevItems = dataView.getItems();
      let numberStartMap = {};
      for (g of getGroupValues(group)) {
        if (g) {
          numberStartMap[g] = document.getElementById(
            `${g}-auto-number-input`
          ).value;
        }
      }
  ...
  // Show modal
    ...
    for (elt of document.querySelectorAll(".dropdown-menu")) {
      elt.classList.remove("show");
    }
    ...
    for (g of getGroupValues(group)) {
      if (g) {
        document.getElementById(
          "auto-number-modal-groups-display"
        ).innerHTML += `
    ...
    document.getElementById(
      "auto-number-modal-group-display"
    ).innerText = `\xa0${group}`;
...
getGroupValues = (group) => {
  ...
  return groups.sort((a, b) => {
    return stableSort(a, b, true, group);
  });
};
...
function executeAutoNumberData() {
  let items = dataView.getItems().sort((a, b) => {
    return stableSort(a, b, true, this.group);
  });
  let countMap = {};
...
undoCellEdit = () => {
  ...
  if (undoRedoBuffer.commandCtr <= 0) {
    return;
  }
  undoRedoBuffer.commandCtr--;
  ...
  if (editor) {
    ...
    if (editor.args) {
      ...
      if (newItem[field] != item[field])
        updateItemCustom(
          dv.getRowById(item.id),
          grid.getColumns().indexOf(editor.args.column),
          newItem,
          item
        );
      editor.destroy();
...
toggleAddColumnModal = (show, direction) => {
  ...
  // Show modal
    ...
    if (["Crew", "Rentals", "Purchases"].includes(_args.section)) {
      document.getElementById(
        "add-column-modal-tax-column-selector"
      ).style.display = "flex";
    }
  }
};
...
addColumn = () => {
  ...
  if (
    grid
      .getColumns()
      .find((c) => c.name == document.getElementById("add-column-input").value)
  ) {
    // GIVE FEEDBACK
    return;
  }
  ...
  command.islongtext = document.getElementById(
    "add-column-editor-selector"
  ).checked;
  command.istaxcolumn = document.getElementById(
    "add-tax-column-editor-selector"
  ).checked;
...
async function executeAddColumn() {
  ...
  if (this.istaxcolumn) {
    ...
    let editor = "text";
    if (this.islongtext) {
      newColumn.editor = Slick.Editors.LongText;
      editor = "longtext";
    }
...
async function undoDeleteColumn() {
  ...
  if (this.column.istaxcolumn) {
    ...
    let editor = "text";
    if (this.column.editor == Slick.Editors.LongText) {
      editor = "longtext";
    }
...
countTotalsFormatter = (totals, columnDef) => {
  ...
  var ttl = 0;
  if (val != null) {
    ttl = new Intl.NumberFormat().format(
      Math.round(parseFloat(val) * 100) / 100
    );
  }
...
toggleValidationModal = (show, validator = null) => {
  ...
  // Show modal
    ...
    document.getElementById("validation-modal-header").innerText =
      validator.validationHeader;
    document.getElementById("validation-modal-message").innerText =
    ...
    if (!_accessProfile.columnFilter.filter.includes(field)) {
      if (cols[args.cell].field == field) {
        if (validator.isInvalid(args, field)) {
          ...
          if (!invalidCells[args.row]) {
            invalidCells[args.row] = {};
          }
          invalidCells[args.row][grid.getColumns()[args.cell].field] =
            "invalid-cell";
...
setWeekEnding = () => {
  ...
  if (_args.section == "Estimate") {
    ...
    document.getElementById(
      "week-ending-display"
    ).innerText = `Estimate Version: ${_version}`;
    document.getElementById("week-ending-display-container").onclick =
      function () {
        toggleOpenVersionModal(true);
      };

    // Add latest indicator if in latest version
    if (_version == _sortedVersionKeys[0]) {
      document.getElementById("week-ending-display-container").style.color =
        "grey";
      document.getElementById("week-ending-display-container").innerHTML +=
        "&nbsp- Latest";
    }
  } else {
    let weekEnd = new Date(_args.weekend);
    let weekEndingText = `Week ${
      _args.weekList.indexOf(
        _args.weekList.find((w) => w._id == _args.weekid)
      ) + 1
    } (Ending: ${weekEnd.toLocaleDateString("en-US")})`;
    if (_args.weekList[_args.weekList.length - 1]._id == _args.weekid) {
      document.getElementById("week-ending-latest-indicator").style.display =
        "flex";
    }
...
toggleDeleteWeekWarningModal = (show, weekNum, weekId, deleteWeek = false) => {
  if (!show) {
    ...
    if (deleteWeek) {
      weekNum = parseFloat(
        document.getElementById("delete-week-warning-modal-number").innerText
      );
      weekId = document.getElementById(
        "delete-week-warning-modal-storage"
      ).innerText;
      _deletedWeek = weekId;
      ...
      _args.weekList.splice(
        _args.weekList.indexOf(_args.weekList.find((w) => w._id == weekId)),
        1
      );
    ...
    document.getElementById("delete-week-warning-modal-number").innerText =
      weekNum;
    document.getElementById("delete-week-warning-modal-storage").innerText =
      weekId;
  }
};
...
toggleWeekEndingModal = (show) => {
  ...
  // Show modal
    ...
    for (i in _args.weekList) {
      ...
      if (week._id == _args.weekid) {
        weekDivStyle = "text-decoration: underline;";
      }

      // Add delete button to weeks if there is more than one week
      let deleteButton = "";
      if (_args.weekList.length > 1 && _args.apOptions["Edit Weeks"]) {
        deleteButton = `<button style='color: red' onclick='toggleDeleteWeekWarningModal(true, ${
          parseInt(i) + 1
        }, "${week._id}")'>Delete</button>`;
      }

      wemWC.innerHTML =
        `
            <div class="week-ending-modal-week" onclick="changeWeek('${
              week._id
            }')" style="${weekDivStyle}">
                Week ${parseInt(i) + 1} (Ending: ${new Date(
          week.end
...
changeWeek = async (
  ...
  weekEnd = false,
  isNewWeek = false,
  copyCrewFrom = "current"
) => {
  if (weekId == _args.weekid && _deletedWeek != _args.weekid) {
    return;
  }
  if (_args.section != "Estimate") {
  ...
  if (!show) {
    if (update) {
      let weekEnd = new Date(
        (document.getElementById("add-week-input").value + "T00:00:00")
          .replace(/-/g, "/")
          .replace(/T.+/, "")
      );
      if (weekEnd == "Invalid Date") {
        document.getElementById("add-week-modal-input-warning").style.display =
          "flex";
        return;
      }
      let copyCrewFrom = "";
      if (document.getElementById("add-week-radio-current").checked) {
        copyCrewFrom = "current";
      }
      if (document.getElementById("add-week-radio-preceding").checked) {
        copyCrewFrom = "preceding";
      }
      if (document.getElementById("add-week-radio-blank").checked) {
        copyCrewFrom = "blank";
      }
...
calculateDailyLaborCost = (multipliers, hours, rate, day) => {
  ...
  // Sort multipliers
  let multiplierKeys = Object.keys(multipliers).sort((a, b) => {
    return a - b;
  });
      ...
      if (h == cq[ctr - 1].destH && w == cq[ctr - 1].destW) {
        ...
        for (let i = 0; i < cq[ctr - 1].destH; i++) {
          for (let j = 0; j < cq[ctr - 1].destW; j++) {
            cq[ctr - 1].oldValues[i][j] =
              cq[ctr - 1].previousItems[i][`${editedFields[j]}`];
          }
        }
      }
    } else {
...
clearDeletedItems = () => {
  let items = dataView.getItems();
  let reqForSaveCols = grid
    .getColumns()
    .filter((c) => c.cssClass && c.cssClass.includes("required-for-save"));
  for (item of items) {
...
addDepartmentCssClass = (d, newColor = false) => {
  if (newColor) {
    document
      .getElementById(`${d.replaceAll(" ", "")}_cssClassElement`)
      .remove();
  ...
  departmentStyle.innerHTML = `.${d.replaceAll(
    " ",
    ""
  )}_cssClass {background: ${color} !important; color: ${textColor} !important;}`;
...
getHideColumnsOptions = (args = _contextCell) => {
  ...
  if (grid.getColumns()[args.cell].isHidden) {
    options = `<li onclick='hideColumns(false, [${_contextCell.cell}, ${_contextCell.cell}])'>Show column</li>`;
  } else {
    options = `<li onclick='hideColumns(true, [${_contextCell.cell}, ${_contextCell.cell}])'>Hide column</li>`;
  }
...
zeroNanToNull = (val) => {
  if (val == 0 || isNaN(val) || val == Infinity || val == -Infinity) {
    val = null;
  }
  return val;
};
...
getGroupedRow = (row) => {
  ...
  for (let i = 0; i < groups.length; i++) {
    ...
    for (let j = 0; j < groups[i].rows.length; j++) {
      if (groups[i].rows[j].id == item.id) {
        return rowCount;
      }
      rowCount++;
      if (j == groups[i].rows.length - 1) {
        rowCount++;
      }
    }
  }
...
toggleAutoFillRangeModal = (show, execute = false) => {
  ...
  if (!show) {
    ...
    if (execute) {
      ...
      let incrementInput = document.getElementById(
        "auto-fill-range-input-increment"
      );
...
reorderDepartments = function () {
  ...
  let nonDeptCols = columns.filter((c) => {
    for (d of depts) {
      if (c.field && (c.field.includes(d) || c.field == "___ExtraColumn___")) {
        return false;
      }
    }
    return true;
  ...
  if (xtraCol) {
    newColumns = newColumns.concat(xtraCol);
  }
...
runAllValidators = () => {
          ...
          for (field of v.fields) {
            ...
            if (!_accessProfile.columnFilter.filter.includes(field)) {
              ...
              if (v.isInvalid(args, field)) {
                ...
                scrollToCell = {
                  row: row,
                  cell: cols.indexOf(cols.find((c) => c.field == field)),
                };
              } else if (
                invalidCells[row] &&
                invalidCells[row][field] &&
                invalidCells[row][field].includes("invalid-cell")
              ) {
  ...
  if (scrollToCell) {
    ...
    if (selectedRange) {
      if (
        scrollToCell.row >= selectedRange.fromRow &&
      ...
      for (col of requiredCols) {
        if (!item[col.field] && !parseInt(item[col.field]) == 0) {
          ...
          grid.scrollCellIntoView(
            dataView.getRowByItem(item),
            grid.getColumns().indexOf(col)
          );
          return true;
        }
      }
...
toggleBlankRequiredWarningModal = (show, save = false) => {
  if (show) {
    document.getElementById("grid-modal-container").style.display = "flex";
    document.getElementById("blank-required-warning-modal").style.display =
      "flex";
  } else {
    document.getElementById("blank-required-warning-modal").style.display =
      null;
...
hideRestrictedColumns = (columns, IDkey) => {
  ...
  if (_args.section == "Estimate" && !_sortedVersionKeys.includes(_version)) {
    return [];
  }

  // initialize hidden columns display setting if it is empty
  if (!_displaySettings.setHiddenColumns) {
    _displaySettings.setHiddenColumns = [];
  }
  ...
  if (isWhitelist) {
    _displaySettings.setHiddenColumns =
      _displaySettings.setHiddenColumns.concat(columns.map((c) => c.field));
  ...
  for (col of columns) {
    if (_accessProfile.columnFilter.filter.includes(col.field)) {
      if (isWhitelist) {
        _displaySettings.setHiddenColumns.splice(
          _displaySettings.setHiddenColumns.indexOf(col.field),
          1
        );
  ...
  if (_displaySettings.setHiddenColumns.includes(IDkey)) {
    _displaySettings.setHiddenColumns.splice(
      _displaySettings.setHiddenColumns.indexOf(IDkey),
      1
    );
...
populateAccessProfileModal = (
  ...
  let accessProfileAccordion = document.getElementById(
    "access-profiles-accordion"
  );
  ...
  for (ap in _args.accessProfiles) {
    ...
    if (_args.accessProfiles[ap].accessLevel <= _args.accessLevel) {
      continue;
    }
    ...
    if (ap == showAp) {
      isCurrentProfile = "show";
    } else if (ap == _args.accessProfileName && !showAp) {
      isCurrentProfile = "show";
    }
    ...
    for (option in _args.accessProfiles[ap].options) {
      ...
      let value = "";
      if (_args.accessProfiles[ap].options[option]) {
        value = "checked";
      }
      apAccordionItem += `
    ...
    for (uName of Object.keys(_args.accessMap).filter(
      (key) => _args.accessMap[key].profile == ap
    )) {
    ...
    for (apPage in _args.accessProfiles[ap]) {
      ...
      if (["options", "accessLevel"].includes(apPage)) {
        continue;
      }
      ...
      if (apPage == showPage) {
        isCurrentSection = "show";
      } else if (apPageName == _args.section && !showPage) {
        isCurrentSection = "show";
      }
      ...
      subAccordionItem += generateDataFilterHtml(
        _args.accessProfiles[ap][apPage].dataFilter,
        ap,
        apPage,
        apName,
        apPageName,
      ...
      subAccordionItem += generateColumnFilterHtml(
        _args.accessProfiles[ap][apPage].columnFilter,
        ap,
        apPage,
        apName,
        apPageName,
      ...
      // Add items for dataFilter
      subAccordionItem += generateDataFilterHtml(
        _args.accessProfiles[ap][apPage].editDataFilter,
        ap,
        apPage,
        apName,
        apPageName,
      ...
      // Add items for columnFilter
      subAccordionItem += generateColumnFilterHtml(
        _args.accessProfiles[ap][apPage].editColumnFilter,
        ap,
        apPage,
        apName,
        apPageName,
...
toggleDataFilterModal = (
  ...
  if (show) {
    ...
    document.getElementById("access-profiles-modal").style.display = null;
    document.getElementById("data-filter-modal-memory").innerText =
      JSON.stringify({ ap, apPage, filterCol, newFilter });
    document.getElementById("data-filter-modal-memory-filterKey").innerText =
      filterKey;
    ...
    if (_args.accessProfiles[memory.ap][memory.apPage][filterKey].type == "w") {
      filterListTypeText = "only";
    }
    if (filterKey == "editDataFilter") {
      filterActionText = "edit";
    }
    ...
    if (newFilter) {
      ...
      document.getElementById("data-filter-column-input").style.display =
        "flex";
      document.getElementById("data-filter-column-input-label").style.display =
        "flex";
      ...
      document
        .getElementById("data-filter-modal-delete")
        .setAttribute("onclick", "toggleDataFilterModal(false)");
    } else {
      let currentValues =
        _args.accessProfiles[memory.ap][memory.apPage][filterKey].filter[
          filterCol
        ].join(", ");
    ...
    document
      ...
      .setAttribute(
        "onclick",
        "toggleDataFilterModal(false,false,false,false,false,false,true)"
      );

    // Get filterKey (determines which filter gets edited) from the modal memory div
    filterKey = document.getElementById(
      "data-filter-modal-memory-filterKey"
    ).innerText;

    // Save data filter filter
    if (save) {
      // Trim all whitespaces at beginning and end of value list, as well as after commas
      let newValues = cleanUpFilterModalInputValues(
        document.getElementById("data-filter-modal-input").value
      );
      // Handle new filter case
      if (memory.newFilter) {
        // Get column name from input
        filterColName = document.getElementById(
          "data-filter-column-input"
        ).value;
        while (filterColName[0] == " ") {
          filterColName = filterColName.slice(1);
        }
        while (
          filterColName[filterColName.length - 1] == " " &&
          filterColName.length >= 2
        ) {
          filterColName = filterColName.slice(0, filterColName.length - 1);
        }
        ...
        let filter =
          _args.accessProfiles[memory.ap][memory.apPage][filterKey].filter;
        if (filter[filterColName]) {
          filter[filterColName] = filter[filterColName].concat(
            newValues.split(",")
          );
        } else {
          filter[filterColName] = newValues.split(",");
        }
      }
      ...
      document
        .getElementById(`${apName}-${apPageName}-dataFilter-${filterColName}`)
        .remove();
      delete _args.accessProfiles[memory.ap][memory.apPage][filterKey].filter[
        memory.filterCol
      ];
    }
...
toggleColumnFilterModal = (
  ...
  if (show) {
    ...
    document.getElementById("access-profiles-modal").style.display = null;
    document.getElementById("column-filter-modal-memory").innerText =
      JSON.stringify({ ap, apPage });
    document.getElementById("column-filter-modal-memory-filterKey").innerText =
      filterKey;
    ...
    // Get filterKey (determines which filter gets edited) from the modal memory div
    filterKey = document.getElementById(
      "column-filter-modal-memory-filterKey"
    ).innerText;
    ...
    if (save) {
      ...
      while (newColName[0] == " ") {
        newColName = newColName.slice(1);
      }
      while (
        newColName[newColName.length - 1] == " " &&
        newColName.length >= 2
      ) {
        newColName = newColName.slice(0, newColName.length - 1);
      }

      _args.accessProfiles[memory.ap][memory.apPage][filterKey].filter.push(
        newColName
      );
    }
...
generateColumnFilterHtml = (
  ...
  if (columnFilter.type == "w") {
    columnFilterStyle = 'style="background-color: white; color: black;"';
  }
...
deleteColumnFilterEntry = (eltId, colName, ap, apPage, filterKey) => {
  ...
  _args.accessProfiles[ap][apPage][filterKey].filter = _args.accessProfiles[ap][
    apPage
  ][filterKey].filter.filter((col) => col != colName);
};
...
toggleAddUserToApModal = (show, ap = false, save = false) => {
  if (show) {
    document.getElementById("add-user-to-ap-modal").style.display = "flex";
    document.getElementById("access-profiles-modal").style.display = null;
    document.getElementById("add-user-to-ap-modal-memory").innerText =
      JSON.stringify({ ap });
    document.getElementById(
      "add-user-to-ap-modal-message"
    ).innerText = `Start typing username and select name from\nlist to add user to '${ap}':`;
    ...
    fetch(server + `/shows/${_args.showid}/${_args.section}/getDropdownNames`, {
      ...
      .then((data) =>
        autocomplete(
          document.getElementById("add-user-to-ap-modal-input"),
          data.dropdownNames
        )
      );
  } else {
    document.getElementById("add-user-to-ap-modal").style.display = null;
    document.getElementById("access-profiles-modal").style.display = "flex";

    if (save) {
      let uName = document.getElementById("add-user-to-ap-modal-input").value;
      uName = uName.slice(uName.indexOf("[") + 1, uName.length - 1);
      ap = JSON.parse(
        document.getElementById("add-user-to-ap-modal-memory").innerText
      ).ap;
      ...
      if (uName != _args.username) {
        ...
        if (!_args.accessMap[uName]) {
          _args.accessMap[uName] = {
            ...
            estimateVersion:
              _args.accessMap[_args.username.replaceAll(".", "-")]
                .estimateVersion,
            profile: ap,
          };
        }
...
toggleAddAccessProfileModal = (show, save) => {
  if (show) {
    ...
    let apKeys = Object.keys(_args.accessProfiles).filter(
      (k) => _args.accessProfiles[k].accessLevel > _args.accessLevel
    );
    ...
    if (save) {
      ...
      if (!newProfileLevel || newProfileLevel <= _args.accessLevel) {
        newProfileLevel = _args.accessLevel + 1;
      }
      let newAccessProfile;

      if (copyProfile != "Create New Profile") {
        newAccessProfile = JSON.parse(
          JSON.stringify(_args.accessProfiles[copyProfile])
        );
      } else {
        newAccessProfile = {
...
parseApCheckboxes = () => {
  for (ap in _args.accessProfiles) {
    ...
    for (opt in profile.options) {
      ...
      let optionElt = document.getElementById(
        `${apName}-${optionName}-checkbox`
      );
      if (optionElt) profile.options[opt] = optionElt.checked;
    }

    for (page of showPages) {
      apPageName = page.replaceAll(" ", "");
      let optionElt = document.getElementById(
        `${apName}-${apPageName}-pageAccess-checkbox`
      );
      if (optionElt) profile[page].pageAccess = optionElt.checked;
    }
  }
};
...
toggleDeleteAccessProfileModal = (show, ap = false, del = false) => {
  if (show) {
    document.getElementById("delete-access-profile-modal").style.display =
      "flex";
    document.getElementById("access-profiles-modal").style.display = null;
    document.getElementById("delete-access-profile-modal-memory").innerText =
      ap;
    document.getElementById(
      "delete-access-profile-modal-message"
    ).innerText = `Deleting access profile '${ap}' will cause all users who are still assigned to this access profile to lose access to the show until they are added to another access profile.`;
  } else {
    document.getElementById("delete-access-profile-modal").style.display = null;
    document.getElementById("access-profiles-modal").style.display = "flex";
    let ap = document.getElementById(
      "delete-access-profile-modal-memory"
    ).innerText;
...
updateApSaveStatus = (saved) => {
  ...
  if (!_args.apOptions["Edit Access Profiles"]) {
    return;
  }
  _accessProfilesSaved = saved;
  if (saved) {
    document.getElementById("access-profile-display").innerText =
      _args.accessProfileName;
    document.querySelector("#access-profiles-modal-message h4").innerText =
      "Access Profiles";
  } else {
    document.getElementById(
      "access-profile-display"
    ).innerHTML = `${_args.accessProfileName}<sup style="font-size: 1.2rem; color: red;">*</sup>`;
    document.querySelector("#access-profiles-modal-message h4").innerText =
      "Access Profiles (unsaved)";
  }
};
...
revertAccessProfile = (ap) => {
  ...
  _args.accessProfiles[ap] = JSON.parse(
    JSON.stringify(_initialAccessProfiles[ap])
  );
...
toggleApLevelModal = (show, ap = false, save = false) => {
  if (show) {
    ...
    if (save) {
      ...
      if (newLevel <= _args.accessLevel) {
        newLevel = _args.accessLevel + 1;
      }
...
preSaveProcedure = (reload) => {
  ...
  if (
    _savingUnderway ||
    (!_overrideBlankRFSWarning && blankRequiredWarning())
  ) {
    return false;
  } else {
    _savingUnderway = true;
  }
