...
toggleAddWeekModal=(show, update) => {
    ...
    if (!show) {
        if (update) {
            let weekEnd=new Date((document.getElementById('add-week-input').value+'T00:00').replace(/-/g, '\/').replace(/T.+/, ''))
            console.log((document.getElementById('add-week-input').value+'T00:00').replace(/-/g, '\/').replace(/T.+/, ''))
            console.log(weekEnd)
...
toggleAddUserToApModal=(show, ap=false, save=false) => {
    if (show) {
        ...
        document.getElementById('access-profiles-modal').style.display=null;
        document.getElementById('add-user-to-ap-modal-memory').innerText=JSON.stringify({ ap })
        ...
        if (save) {
            ...
            ap=JSON.parse(document.getElementById('add-user-to-ap-modal-memory').innerText).ap
